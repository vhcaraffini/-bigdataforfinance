...
Python Postgres Streamlit Jupyter Gitmoji
...
Cadastro de Cias Abertas	Dados cadastrais (CNPJ, Situação, Setor, Governança)	🔗
DFP (Demonstrações Financeiras Padronizadas)	BP, DRE, DFC — dados anuais	🔗
ITR (Informações Trimestrais)	BP, DRE, DFC — dados trimestrais	🔗
FRE (Formulário de Referência)	Governança, controle acionário, riscos	🔗
📂 Diretório completo de arquivos CVM: http://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/
...
Python 3.10+
PostgreSQL — instale e deixe rodando localmente
VS Code com as extensões Python e Jupyter
Git
2. Crie o projeto localmente e publique no GitHub via VS Code
Crie a pasta bigdata_for_finance dentro de Documentos
Abra o VS Code e use File → Open Folder para abrir essa pasta
No painel Source Control (Ctrl+Shift+G), clique em "Initialize Repository"
Faça o primeiro commit (ex: 🎉 Iniciando o projeto)
Clique em "Publish Branch" — o VS Code cria o repositório no GitHub e faz o push inicial automaticamente
...
queries/
dashboard/
Consulte a seção 📁 Estrutura do Projeto para entender o propósito de cada pasta.
...
Safe Mode na Curadoria Vertical: Nunca sobrescreva um valor original da CVM se abs(valor) > 0.01. Reconstrução de pais só preenche lacunas (nulos/zeros).

IS_LEAF obrigatório: Toda tabela Silver deve ter a coluna IS_LEAF corretamente calculada.

Deduplicação blindada: Sempre use ROW_NUMBER() particionado por (CNPJ, DT_REFER, CD_CONTA, DS_CONTA) ordenado por VERSAO DESC para garantir a versão mais recente.

SQL com CTEs: Prefira WITH nome_cte AS (...) para legibilidade. Use aspas duplas "NOME_COLUNA" no PostgreSQL.

Logs legíveis: Use emojis nos print() para demarcar etapas do pipeline:
